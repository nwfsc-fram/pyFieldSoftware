...
class Notes(QObject):
    ...
    def get_next_note_name(self):
        ...
        logging.info(f"next_path = {next_path}")

        return next_path

    @pyqtSlot(str, str, str, QVariant, QVariant, QVariant, QVariant, QVariant, name="insertNote")
    def insert_note(self, app_name, screen, value, site_op_id=None, drop=None, angler=None, hook=None,
        ...
        try:
            ...
            sql = """
                INSERT INTO NOTES(APP_NAME, SCREEN_NAME, NOTE, OPERATION_ID, HL_DROP, HL_ANGLER, HL_HOOK, DATE_TIME,
                    IMAGE_NAME_STR, IMAGE_NAME_BLOB)
                    VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?);
            """
            date_time = arrow.now().to("US/Pacific").isoformat()
            # Read in the image into memory for insertion into the blob

            # logging.info(f"imageNameStr = {imageNameStr}")
            if os.path.exists(imageNameStr):
                image = Path(imageNameStr).read_bytes()

            # logging.info(f"image = {image}")

            params = [app_name, screen, value, site_op_id, drop, angler, hook, date_time,
                      imageNameStr, image]

            logging.info(f"app_name={app_name}, screen={screen}, value={value}, "
                         f"site_op_id={site_op_id}, drop={drop}, angler={angler}, hook={hook}, date_time={date_time},"
                         f"imageNameStr={imageNameStr}")
