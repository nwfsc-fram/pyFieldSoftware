...
class SerialPortManager(QObject):
    ...
    dataReceived = pyqtSignal(str, str, str, bool, arguments=["action", "value", "uom", "dbUpdate"])
    ...
    def data_received(self, com_port, measurement, data, value):
        ...
        logging.info(f"Emitting tab update from serial port: {com_port}, {currentEntryTab}, {value}, {measurement}")
        ...
        try:
            ...
            if idx != - 1:
                ...
                if (measurement_name == "Weight" and currentEntryTab == "weight") or \
                    ...
                    self.dataReceived.emit(currentEntryTab, value, "", False)
        ...
        if screen == "weighbaskets":
            ...
            if measurement == "Barcode (number)":
                try:
                    ...
                    logging.info('isCoral: ' +
                                 str(self._app.process_catch.checkSpeciesType("coral", self._app.state_machine.species["taxonomy_id"])))
                    logging.info('rowIndex: ' + str(self._app.special_actions.rowIndex) + ', widgetType: ' + \
                                 str(self._app.special_actions.rowWidgetType))

                    if self._app.process_catch.checkSpeciesType("coral", self._app.state_machine.species["taxonomy_id"]):
                        # Coral - so find all of the sibling coral items, and in particular find the Coral Specimen ID item
                        ...
                        if rowIndex != -1:
                            ...
                            for i in range(model.count):
                                if model.get(i)["parentSpecimenNumber"] == parent_specimen_number and \
                                                model.get(i)["specialAction"] == "Coral Specimen ID":
