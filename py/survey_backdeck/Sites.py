        ...
        sql = """
            SELECT o.operation_number, s.name, o.area, e.event_type_lu_id, 
                CASE WHEN e.start_date_time THEN e.start_date_time
                     ELSE o.date END AS date_time, o.operation_id, o.cs_processing_status              
            FROM OPERATIONS o 
...
class Sites(QObject):
    ...
    def finished_and_validate(self):
        ...
        try:
            # Update the database
            sql = """
                UPDATE OPERATIONS SET CS_PROCESSING_STATUS = 'finished'
                WHERE OPERATION_ID = ?
            """
    ...
    def reopen_site(self, model_index, site_op_id):
        ...
        try:
            ...
            sql = """
                UPDATE OPERATIONS SET CS_PROCESSING_STATUS = NULL
                    WHERE OPERATION_ID = ?;
            """
            params = [site_op_id, ]
            self._app.rpc.execute_query(sql=sql, params=params)
