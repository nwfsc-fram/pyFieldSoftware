...
//                  locationData[i]["position"] + ' depth ' + locationData[i]["depth"]);
    ...
    GridLayout {
        ...
        function editLocation(loc_id, pos_num) {
            ...
            dlgGPS.haul_num = parseInt(gridHaulRow.set_num);
        ...
        function newLocation(pos_num) {
            ...
            var sub_title = "Set " + gridHaulRow.set_num + ": New ";
            if (pos_num === -1)
                sub_title += "Set Entry";
            else if (pos_num === 0)
                sub_title += "Up Entry";
            ...
            console.log("New location for Set #" + dlgGPS.haul_num)
            dlgGPS.open();
        }
            ...
            RowLayout {
                ...
                MouseArea {
                    ...
                    onClicked: {
                        ...
                        dlgGPS.location_id = loc_id;
                        dlgGPS.sub_title = "Edit Set #" + gridHaulRow.set_num + ": " + rowname;
...
// -------
    ...
    GridLayout {
        ...
        Repeater {
            id: repeatObsRet
            model: appstate.sets.getObserverRetModel(gridHaulRow.set_db_id)
        ...
        Repeater { // if some other row has more Obs Ret entries, insert blanks
            ...
            function calc_model_length() {
                return appstate.sets.maxObsRetModelLength - repeatObsRet.model_count;
            }
    ...
    Connections {
        ...
        onAccepted: {
            // string vs number
            if (parseInt(gridHaulRow.set_num) !== parseInt(dlgGPS.haul_num)) {
                //console.log("IGNORED - not our set " + gridHaulRow.set_num + " vs " + dlgGPS.haul_num);
                return;
            }
            ...
            if (dlgGPS.new_entry) {
                dlgGPS.new_entry = false;
                appstate.sets.locations.add_update_location_haul_id(
                            gridHaulRow.set_db_id,
                            position_number,
                ...
                appstate.sets.locations.update_location_by_id(
                            location_id,
                            CommonUtil.get_date_str(time_val),
                            lat_degs, // latitude
                            long_degs, // longitude
    ...
    Connections {
        ...
        onCc_codeChanged: {
            ...
            if (gridHaulRow.set_num !== dlgCCPicker.haul_num) {
            ...
            if (dlgCCPicker.cc_code !== "") {
                ...
                if (appstate.sets.ccIsInObserverRetModel(gridHaulRow.set_db_id, dlgCCPicker.cc_code)) {
        ...
        onAccepted: {
            ...
            var set_db_id = gridHaulRow.set_db_id;
            if (!repeatVesselRet.model) {  // not sure why this gets cleared
                repeatVesselRet.model = repeatVesselRet.get_vessel_retained(set_db_id);
            }
            appstate.sets.addVesselRetained(set_db_id, new_elem);
            console.log("Added Vessel Ret entry to " + set_db_id);
            dlgCCPicker.clear();
        }
    }
