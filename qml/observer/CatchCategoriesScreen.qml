...
Item {
    ...
    signal catchCatsDetailsCompleted
    onCatchCatsDetailsCompleted: {
            console.debug("CCDetailsScreen signaled: details completed.");
    ...
    ListView
    {
        ...
        Column {
            ...
            function addCatchCat() {
                ...
                if (newElem === undefined || newElem.catch_category === undefined) {
                    ...
                    var parentWindow = screenCCEntry;
        ...
        ObserverTableView {
            ...
            function activate_CC_selected() {
                ...
                btnWeightMethod3Data.visible = screenCCEntry.isWM3();
                console.debug("btnWM3.visible=" + screenCCEntry.isWM3());
            ...
            Connections {
                ...
                // The checks for both catch_weight and catch_count are the same. Use a single function.
                function handleTotalCatchChangedSignal(
                        parmName,   // Either "catch_count" or "catch_weight"
                        parmValue) {
                    ...
                    if (!parmValue || parmValue === null) {
                        console.debug("Received null total " + parmName + "; skipping update.");
                        return;
                    }
                    if (appstate.catches.currentSampleMethodIsNoSpeciesComposition) {
                        console.debug("Received species " + parmName + " data - skipped because CC is NSC");
                        return;
                    }
                    if (!appstate.catches.currentSampleMethodIsSpeciesComposition) {
                        console.debug("Received species " + parmName + " data - skipped because sample method not yet specified.");
                        return;
                    }
                    if (appstate.catches.weightMethod == '3') {
                        console.debug("Received basket " + parmName + " data, but disregarding. " +
                                "WM is 3 - using WM3 catch-level basket weight data");
                        return;
                    }
                    if (appstate.catches.weightMethod == '14') {
                        console.debug("Received basket " + parmName + " data, but disregarding. " +
                                "Showing 'Visual Experience' weight.");
                        return;
                    }
//                    console.debug("Connection in tvSelectedCatchCat received total " + parmName + " change signal; " +
//                            "persisting total " + parmName + " of " + parmValue + " to database.");
                    appstate.catches.setData(parmName, parmValue);
                }

                onTotalCatchWeightChanged: {    // Parameter: weight
                    handleTotalCatchChangedSignal("catch_weight", weight);
                }
                onTotalCatchCountChanged: {     // Parameter: count
                    handleTotalCatchChangedSignal("catch_count", count);
                }
            }
            ...
            Connections {
                ...
                onTotalPHLBWeightChanged: {
                    if (weight) {
                        appstate.catches.setData("catch_weight", weight);
                    }
                }
            }
            onCurrentRowChanged:{
            ...
            function editItemDetails(item_index) {
                var detailProps = {
                    ...
                    'ccModel': model,
                    'ccIndex': item_index,
                    'ccScreenId': screenCCEntry}
                var ccdetailsPage =
                        stackView.push(
                            {item: Qt.resolvedUrl("CatchCategoriesDetailsScreen.qml"),
