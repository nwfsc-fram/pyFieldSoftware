// TripDetailsScreen.qml - Start or Edit Trip Details (Trawl)
...
Item {
    ...
    ////
    // Validation: Permits/Licenses
    ////

    // Formats for GF (GroundFish) Permit or License:
    // 1. 5 digits              (\d{5})
    // 2. "GF" plus 4 digits    (GF\d{4})
    // 3. "L" plus 5 digits     (L\d{5})
    // 4. "BT" plus 4 digits    (BT\d{4})
    function permit_or_license_parses(per_or_lic_text) {
        return (/^((\d{5})|(GF\d{4})|(L\d{5})|(BT\d{4}))$/.test(per_or_lic_text));
    }
