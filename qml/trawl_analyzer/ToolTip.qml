...
Item {
    ...
    Component.onCompleted: {
        ...
        // If a TextField, change the MouseArea cursor to an IBeam
        if (itemParent.toString().indexOf("TextField") > -1) {
            var newObject = Qt.createQmlObject('import QtQuick 2.0; MouseArea {signal mouserHover(int x, int y); signal showChanged(bool flag); anchors.fill:parent; cursorShape: Qt.IBeamCursor; hoverEnabled: true; onPositionChanged: {mouserHover(mouseX, mouseY)} onEntered: {showChanged(true)} onExited:{showChanged(false)} onClicked:{parent.focus = true}}',
