...
//    property int age
    ...
    property string action: "Do you want to keep this new value?"
    property string valueLabel: (typeof value == "number") ? value : (value == undefined) ? "" : value

//    standardButtons: StandardButton.Ok | StandardButton.Cancel

    signal setvalue(var newvalue)
    onSetvalue: {
        if (measurement == "Length")
            value = (typeof newvalue == 'number') ? parseFloat(newvalue).toFixed(1) : null
        else if (measurement == "Weight")
            value = (typeof newvalue == 'number') ?  parseFloat(newvalue).toFixed(3) : null
        else if (measurement == "Age Barcode ID")
            value = newvalue;
    }
...
//        color: SystemPaletteSingleton.window(true)
        ...
        Label {
            ...
            x: 20
            y: 20
            text: measurement + ":   " + valueLabel + " " + unit_of_measurement
            font.pixelSize: 20
        } // lblMeasurementValue
        Label {
            id: lblErrorTitle
